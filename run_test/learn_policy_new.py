...
def get_log_prob(rlt):
    ...
    edge_index = edge_index.squeeze().detach().numpy()
...
def get_batch_reward_loss_new(output, cand_pids, pid_batch, option_batch, unit_batch, label_batch, args):
...
def policy_gradient_train_new(device,problems, train_pids, cand_pids, cand_examples, args):
    ...
    policy_model = PolicyNet(data, model_config="bert-uncased-small", add_linear=False, device = device)
    ...
    for epoch in range(args.epochs):
        ...
        for batch_i in range(num_batch):
            ...
            cids, reward, loss = get_batch_reward_loss_new(rlt, cand_pids, pid_batch, option_batch, unit_batch,
                                                       label_batch, args)
