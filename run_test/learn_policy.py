...
def load_data(args):
    ...
    problems = json.load(open("../data/alpaca-train-data.json"))
...
def get_batch_reward_loss(scores, cand_pids, pid_batch, option_batch, unit_batch, label_batch, args, sim_model):
...
def policy_gradient_train(policy_model, problems, train_pids, cand_pids, cand_examples, args, sim_model):
    # REINFORCE
...
if __name__ == '__main__':
    ...
    sim_model = sim_cal_model("/data/model/bert-base-uncased").to(torch.device("cuda:0"))
    ...
    policy_gradient_train(policy_model, problems, train_pids, cand_pids, cand_examples, args, sim_model)
