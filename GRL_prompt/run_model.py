...
def parse_args():
    ...
    parser.add_argument('--test_number', type=int, default=100, help='GPT-4 is expensive. -1 for the whole test set')
...
if __name__ == '__main__':
    ...
    if args.ckpt:
        ...
        emb_path = os.path.join(args.ckpt_root, args.ckpt, "emb_best_reward.pt")
        ...
        for component in [emb_path, gnn_path, linkpred_path, selectn_path]:
            if not os.path.exists(component):
                ...
                exit()
        policy_model.emb.linear.load_state_dict(torch.load(emb_path))
    ...
    with torch.no_grad():
        for i, pid in enumerate(pids):
            ...
            example = create_example_from_pid(pid, problems, args, test=True)
            rlt = policy_model([example], cand_examples)

            prompt_sorted,log_prob = get_log_prob(rlt, 0)
