...
class Graph:
    ...
    def get_all_edges(self):
        return [(k,ele[0]) for k,v in self.graph.items() for ele in v]
...
def node_map(selected):
    rlt = {selected[i]:i for i in range(len(selected))}
    anti_rlt = {v:k for k,v in rlt.items()}
    return rlt,anti_rlt

def link_idx_map(edges,edge_index):
    num_edge = edges[0].shape[0]
    rlt_index = dict()
    for i in range(num_edge):
        rlt_index[(int(edges[0,i]),int(edges[1,i]))] = i
    return rlt_index


def return_order_prob(rlt,selected):
    link_pred, edge_index, select_score, edge_index_dict = rlt
    num_can = len(selected)
    selected_map,anti_map = node_map(selected)
    # create graph obj
    g = Graph(num_can)
    edges = edge_index_dict[('can','to','can')]
    num_edge = edges[0].shape[0]
    edge_map = link_idx_map(edges,edge_index)
    # read pred label
    for i in range(num_edge):
        pred = bool(edge_index[i])
        if pred:
            log0 = float(torch.log(link_pred[i]))
            pos_s1, pos_s2 = int(edges[0][i]), int(edges[1][i])
            if pos_s1 in selected and pos_s2 in selected:
                g.addEdge(selected_map[pos_s1], selected_map[pos_s2], log0)
    while g.isCyclic():
        print(1)
        g.isCyclic()
    edges_final = g.get_all_edges()
    order = g.topologicalSort()
    order = [anti_map[ele] for ele in order]
    edge_rlt_index = [edge_map[(anti_map[ele[0]],anti_map[ele[1]])] for ele in edges_final]
    return order,edge_rlt_index
